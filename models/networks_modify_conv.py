...
def unet_conv(input_nc, output_nc, norm_layer=nn.BatchNorm2d):
    downnorm_in = norm_layer(input_nc)
    downconv = nn.Conv2d(input_nc, output_nc, kernel_size=4, stride=2, padding=1)
    downrelu = nn.LeakyReLU(0.2, True)
    downnorm = norm_layer(output_nc)
    conv1x1 = create_conv(output_nc, output_nc, 1, 0)
    conv1x1.apply(weights_init)
    return nn.Sequential(*[downnorm_in, downconv, downnorm, downrelu, conv1x1])
...
def unet_upconv(input_nc, output_nc, outermost=False, norm_layer=nn.BatchNorm2d):
    ...
    conv1x1.apply(weights_init)    
    if not outermost:
        return nn.Sequential(*[upconv, upnorm, uprelu, conv1x1])
    else:
