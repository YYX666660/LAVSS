...
class Resnet18(nn.Module):
    def __init__(self, original_resnet, pool_type='maxpool', input_channel=3, with_fc=False, fc_in=512, fc_out=512):
        ...
        # self.conv3 = torch.nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=2, padding=1)
        # self.conv3.apply(weights_init)
        # self.conv4 = torch.nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=2, padding=1)
        # self.conv4.apply(weights_init)
        # self.pool = torch.nn.MaxPool2d(2)

        self.space = SpatialAttention(7)


    def forward(self, x, y):
        x = self.feature_extraction(x)      # [512,7,7]
        x_space = self.space(x)     # [1,7,7]

        # position feature
        y = self.conv1x1_2(y)
        y = y * x_space     # [512,7,7]与visual结合后的pos
