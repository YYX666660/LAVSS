...
class BaseDataset(torchdata.Dataset):
    ...
    def _stft(self, audio):
        ...
        amp = np.abs(spec)
        phase = np.angle(spec)
        return torch.from_numpy(amp), torch.from_numpy(phase), spec
    
    def _stft_torch(self, audio):
        spec = torch.stft(
            torch.from_numpy(audio), n_fft=self.stft_frame, hop_length=self.stft_hop, return_complex=True)
        return spec
    ...
    def _mix_n_and_stft(self, audios):
        ...
        # STFT
        amp_mix, phase_mix, spec_mix = self._stft(audio_mix)
        for n in range(N):
            ampN, _, _ = self._stft(audios[n])
        ...
        return amp_mix.unsqueeze(0), mags, phase_mix.unsqueeze(0), spec_mix, torch.from_numpy(audio_mix)   # amp_mix混合幅值，mags每种乐器单独的幅值,phase_mix混合相位
