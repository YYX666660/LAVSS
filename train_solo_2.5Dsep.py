...
# Our libs
from arguments_solo_music import ArgParser
from dataset_solo_nopos import MUSICMixDataset
...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args):    # 关键主程序，送数据，对应music里的dict数据
        ...
        for n in range(N):
            pred_masks[n] = self.net_unet(log_mag_mix, feat_frames[n])
            pred_masks_r[n] = self.net_unet(log_mag_mix_r, feat_frames[n])
